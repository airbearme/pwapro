...
/**
 * ⚡ Bolt Optimization:
 * 1. Wrapped component in React.memo to prevent unnecessary re-renders when parent state changes but props remain deep-equal.
 * 2. Optimized AirBear count calculation from O(N*M) to O(N+M) using a Map.
 * 3. Implemented in-place marker updates instead of full clear/re-add for both spots and airbears.
 * 4. Used refs for event callbacks to avoid stale closures without requiring effect re-runs.
 */
const MapView = React.memo(function MapView({
  ...
  // Pre-calculate AirBear counts per spot for O(1) lookup during marker render
  const airbearCountsBySpot = useMemo(() => {
  ...
  useEffect(() => {
    ...
    // Track active spot IDs to remove stale ones
    const activeSpotIds = new Set(spots.map((s) => `spot-${s.id}`));

    // Sync spot markers
    spots.forEach((spot) => {
      const markerId = `spot-${spot.id}`;
      const airbearCount = airbearCountsBySpot.get(spot.id) || 0;
      const hasAvailableAirbears = airbearCount > 0;
      ...
      const popupContent = `
        <div style="min-width: 240px; padding: 12px; font-family: system-ui, -apple-system, sans-serif;">
          ...
          <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 12px; padding: 10px; background: ${
            ...
            }; font-size: 15px;">${airbearCount} AirBear${
              airbearCount !== 1 ? "s" : ""
            } available</span>
          </div>
          ${
      ...
      if (marker) {
        ...
        // Setup click handler for booking using ref to avoid stale closures
        marker.on("click", () => {
  ...
  }, [spots, airbearCountsBySpot, mapLoaded]);
  ...
  useEffect(() => {
    ...
    // Track active airbear IDs to remove stale ones
    const activeAirbearIds = new Set(airbears.map((a) => `airbear-${a.id}`));
