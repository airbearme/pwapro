...
// ⚡ Bolt: Consolidated marker styles and animations to avoid redundant style injections.
// This single style block covers all marker types and states.
const MARKER_STYLES = `
  @keyframes pulse {
  ...
  @keyframes airbear-marker-pulse {
    0%, 100% {
...
const MapView = memo(({
  ...
  // ⚡ Bolt: Optimized marker management. Updates markers in-place (O(N+M))
  // and uses dirty checking to avoid redundant Leaflet setIcon calls.
  useEffect(() => {
    ...
    spots.forEach((spot) => {
      ...
      // Dirty checking: Only update icon if the availability state changed
      const iconState = `${hasAvailableAirbears}`;
      ...
      // Dirty checking: Only update popup if content changed
    ...
    // Remove defunct spot markers
    markersRef.current.forEach((marker, id) => {
      if (id.startsWith("spot-") && !spots.some(s => `spot-${s.id}` === id)) {
        marker.remove();
        markersRef.current.delete(id);
      }
    });

    // --- 2. Update/Add AirBear Markers ---
    airbears.forEach((airbear) => {
      const markerId = `airbear-${airbear.id}`;
      let marker = markersRef.current.get(markerId);

      // Update position (fast in Leaflet)
      if (marker) {
        marker.setLatLng([airbear.latitude, airbear.longitude]);
      }

      // Dirty checking: Only update icon if availability or charging state changed
          ...
          html: `
            <div style="position: relative; cursor: pointer; filter: drop-shadow(0 4px 8px rgba(0,0,0,0.3));">
              <div style="
                ...
                animation: ${
                  airbear.is_available
                    ? "airbear-marker-pulse 2s ease-in-out infinite"
                    : "none"
                };
                transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1);
              "
      ...
      // Dirty checking: Only update popup if battery or status changed
      const dataHash = `${airbear.battery_level}-${airbear.is_charging}-${airbear.is_available}`;
      if (marker.__dataHash !== dataHash) {
        const batteryColor =
    ...
    // Remove defunct airbear markers
    markersRef.current.forEach((marker, id) => {
      if (id.startsWith("airbear-") && !airbears.some(a => `airbear-${a.id}` === id)) {
        marker.remove();
        markersRef.current.delete(id);
      }
    });
