...
const MapView = memo(({
  ...
  useEffect(() => {
    ...
    const initMap = async () => {
      try {
        ...
        // Add consolidated animations style block
        if (!styleRef.current) {
  ...
  useEffect(() => {
    ...
    // Track active spots to remove stale ones later
    ...
    spots.forEach((spot) => {
      ...
      const iconState = hasAvailableAirbears ? "available" : "unavailable";

      let marker = markersRef.current.get(markerId);

      // Only regenerate icon if state changed
      if (!marker || marker.__iconState !== iconState) {
          ...
          html: `
            <div style="position: relative; cursor: pointer; filter: drop-shadow(0 4px 8px rgba(0,0,0,0.3));">
              <div style="
                ...
                animation: ${
                  hasAvailableAirbears
                    ? "pulse-glow-marker 2s ease-in-out infinite"
                    : "none"
                };
                overflow: hidden;
              "
      ...
      // Update popup content if data changed
      const dataHash = `${airbearsAtSpot.length}-${spot.name}-${spot.description}`;
  ...
  useEffect(() => {
    ...
    const map = mapInstanceRef.current;
    const L = LeafletRef.current;
    const currentAirbearIds = new Set(airbears.map(a => `airbear-${a.id}`));
    ...
    airbears.forEach((airbear) => {
      ...
      // Only regenerate icon if availability or charging state changed
      if (!marker || marker.__iconState !== iconState) {
        const icon = L.divIcon({
          html: `
            <div style="position: relative; cursor: pointer; filter: drop-shadow(0 4px 8px rgba(0,0,0,0.3));">
              <div style="
                ...
                animation: ${
                  airbear.is_available
                    ? "pulse-glow-airbear 2s ease-in-out infinite"
                    : "none"
                };
                transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1);
              ">
              ...
              ${
                airbear.is_charging
                  ? `<div style="position: absolute; top: -8px; right: -8px; background: linear-gradient(135deg, #fbbf24, #f59e0b); border-radius: 50%; width: 22px; height: 22px; border: 3px solid white; display: flex; align-items: center; justify-content: center; font-size: 12px; box-shadow: 0 4px 12px rgba(251, 191, 36, 0.6); animation: pulse 1.5s ease-in-out infinite;">⚡</div>`
                  : ""
              }
            </div>
          `,
      ...
      // Update popup content if relevant data changed
      const dataHash = `${airbear.battery_level}-${airbear.is_available}-${airbear.is_charging}`;
      if (marker.__dataHash !== dataHash) {
          ...
          <div style="min-width: 220px; padding: 12px; font-family: system-ui, -apple-system, sans-serif;">
            ...
            <div style="display: flex; flex-direction: column; gap: 10px; font-size: 14px;">
              ...
              <div style="display: flex; justify-content: space-between; padding: 8px; background: linear-gradient(135deg, #f9fafb, #f3f4f6); border-radius: 8px; border-left: 4px solid ${airbear.is_available ? "#10b981" : "#6b7280"};">
                ...
                <span style="font-weight: 700; color: ${airbear.is_available ? "#10b981" : "#6b7280"};">
    ...
    // Remove stale airbear markers
    markersRef.current.forEach((marker, id) => {
      if (id.startsWith("airbear-") && !currentAirbearIds.has(id)) {
        marker.remove();
        markersRef.current.delete(id);
      }
    });
