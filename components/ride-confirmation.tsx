...
export function RideConfirmation({
  ...
  const [paymentStep, setPaymentStep] = useState<"pending" | "processing" | "complete">("pending")
  const [rideStep, setRideStep] = useState<"waiting" | "arriving" | "in_progress" | "completed">(
    "waiting"
  )
  ...
  return (
    <div className="max-w-md mx-auto space-y-4">
      ...
      <Card className="relative overflow-hidden">
        <div className={`absolute top-0 left-0 w-full h-1 ${getRideStatusColor()}`} />
        <CardHeader className="text-center">
          ...
          <CardDescription>Ride ID: #{ride.id.slice(-8).toUpperCase()}</CardDescription>
        </CardHeader>
          ...
          <div className="space-y-3">
            <div className="flex items-start space-x-3">
              ...
              <div className="flex-1">
                ...
                {pickupSpot.address && (
                  <p className="text-sm text-muted-foreground">{pickupSpot.address}</p>
                )}
              </div>
            </div>
            ...
            <div className="flex items-start space-x-3">
              ...
              <div className="flex-1">
                ...
                {destinationSpot.address && (
                  <p className="text-sm text-muted-foreground">{destinationSpot.address}</p>
                )}
              </div>
            </div>
          </div>
      ...
      {paymentStep === "pending" && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>Complete payment to confirm your ride</CardDescription>
          </CardHeader>
          <CardContent>
            <CheckoutButton
              items={[
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="space-y-3">
              ...
              <div
                className={`flex items-center space-x-3 ${
                  rideStep === "completed" ? "text-green-600" : "text-muted-foreground"
                }`}
              >
                <div
                  className={`w-4 h-4 rounded-full ${
