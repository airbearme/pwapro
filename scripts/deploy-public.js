...
async function main() {
	...
	try {
		...
		const manifestExists = await sftp.exists(
			joinRemote(REMOTE_DIR, "manifest.json"),
		);
		if (!indexExists || !manifestExists) {
			throw new Error(
				"Deployment verification failed (missing index or manifest)",
			);
		}
