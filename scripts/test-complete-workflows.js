...
async function testAuthenticationWorkflow() {
  ...
  if (loginPage.status === 200) {
    const hasGoogle = loginPage.body.includes("Google") || loginPage.body.includes("google")
    const hasApple = loginPage.body.includes("Apple") || loginPage.body.includes("apple")
...
async function main() {
  ...
  Object.entries(results).forEach(([workflow, passed]) => {
    console.log(`${passed ? "✅" : "❌"} ${workflow}: ${passed ? "PASSED" : "FAILED"}`)
  })
