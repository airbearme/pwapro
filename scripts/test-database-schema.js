...
const requiredTables = {
  ...
  airbears: ['id', 'latitude', 'longitude', 'battery_level', 'is_available', 'is_charging'],
  rides: ['id', 'user_id', 'pickup_spot_id', 'dropoff_spot_id', 'status', 'fare'],
  users: ['id', 'email', 'role'],
  ...
  try {
    ...
    for (const [table, columns] of Object.entries(requiredTables)) {
      ...
      if (error) {
        ...
        console.log(`✅ Table '${table}': valid (${columns.length} columns checked)`);
      }
    }
