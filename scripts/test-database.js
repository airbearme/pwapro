...
async function testDatabase() {
  try {
    ...
    const { data: spots, error: spotsError } = await supabase.from("spots").select("id").limit(1)
    ...
    const { error: errorLogsError } = await supabase.from("error_logs").select("id").limit(1)
