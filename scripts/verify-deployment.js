...
const IONOS_CONFIG = {
	...
	password: process.env.IONOS_SFTP_PASSWORD || "process.env.IONOS_PASSWORD",
	port: 22,
...
async function verifyDeployment() {
	...
	try {
		...
		if (!exists) {
			throw new Error(
				`Remote directory ${IONOS_CONFIG.remoteBase} does not exist!`,
			);
		}
		...
		const criticalFiles = ["index.html", "sw.js", "manifest.json", "assets"];
		...
		if (files.includes("assets")) {
			...
			console.log(
				`📦 Assets: ${jsFiles.length} JS files, ${cssFiles.length} CSS files`,
			);
